...
# 2/6/2020: 4.5/4.6 must be acquired from internal Red Hat CI registry, because it is not yet available 
# from the official mirror
if [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.5" ] || [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.6" ]; then
    ...
    # 2/6/2020: All versions before 4.5/4.6 are available from the official mirror
    ...
    if [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.3" ] || [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.4" ]; then
        # 4.3/4.4 uses one unified metal image 
...
LATEST_4_6="$(curl -sS https://openshift-release-artifacts.svc.ci.openshift.org/ | awk "/4\.6\./ && !(/s390x/ || /ppc64le/)" | tail -1 | cut -d '"' -f 2)"
...
declare -A OCP_BINARIES=(
    ...
    [4.6]="https://openshift-release-artifacts.svc.ci.openshift.org/$LATEST_4_6"
)
...
if [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.5" ] || [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.6" ]; then
    # HACK: 4.5/4.6 has a different HTML structure than the rest
    FIELD_SELECTOR=2
fi
...
if [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.4" ] || [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.5" ] || [ "$OPENSHIFT_RHCOS_MAJOR_REL" == "4.6" ]; then
